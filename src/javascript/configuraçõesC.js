...
async function handlePasswordChange(user, currentPassword, newPassword) {
  ...
  try {
    ...
    if (isGoogleUser) {
      const isCurrentPasswordValid = await verifyCustomPassword(user.uid, currentPassword);
      
      if (!isCurrentPasswordValid) {
        showMessage('Senha atual incorreta.', 'error');
        return;
      }
      await saveCustomPassword(user.uid, newPassword);
      showMessage('Senha personalizada alterada com sucesso!', 'success');
    } else {
...
async function verifyCustomPassword(userId, inputPassword) {
  try {
    ...
    const data = doc.data();
    const storedPassword = data.senhaCustomizada;
    if (!storedPassword) {
    ...
    console.error('Erro ao verificar senha customizada:', error);
    return false;
  }
}

async function saveCustomPassword(userId, password) {
  try {
    await db.collection('usuarios').doc(userId).update({
      senhaCustomizada: password, 
      senhaCustomizadaAtualizada: firebase.firestore.FieldValue.serverTimestamp()
    });
  } catch (error) {
    console.error('Erro ao salvar senha customizada:', error);
    throw error;
  }
}
