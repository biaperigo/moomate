$(document).ready(function () {
  ...
  $('#btn-google').on('click', function () {
    auth.signInWithPopup(googleProvider)
      ...
      .then(() => {
        ...
        window.location.href = "login.html"; // Redireciona para a área logada
      })
      .catch((error) => {
  ...
  // =================================================
  $('.cadastro-form').on('submit', async function (e) { // Usa async/await para código mais limpo
    e.preventDefault();
    ...
    try {
      // PASSO 1: Verificar se o NÚMERO DE TELEFONE já existe no Firestore
      const telefoneSnapshot = await db.collection("usuarios").where("telefone", "==", telefone).get();
      if (!telefoneSnapshot.empty) {
        alert("Este número de telefone já está em uso. Por favor, tente fazer login ou use um número diferente.");
      ...
      await db.collection("usuarios").doc(createdUser.uid).set({
        ...
        email: email,
        criadoEm: firebase.firestore.FieldValue.serverTimestamp(),
        emailVerificado: false // O e-mail começa como não verificado
      });
